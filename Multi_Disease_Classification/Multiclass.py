...
paths = {
    "IBD": r"/Users/user/Downloads/Disease_1.csv",
    "T2D": r"/Users/user/Downloads/Disease_2.csv",
    "CRC": r"/Users/user/Downloads/Disease_3.csv"
}

datasets = {k: load_dataset(v) for k, v in paths.items()}

# --- Align features across studies ---
all_features = sorted(set().union(*[df.index for df, _ in datasets.values()]))
X_list, y_list, batch_labels = [], [], []

for study, (df, y) in datasets.items():
    df_aligned = df.reindex(all_features, fill_value=0)  # Fill NaN with 0
    df_final = df_aligned.T
    X_list.append(df_final)
    y_list.append(pd.Series(y, index=df_final.index))
    batch_labels.extend([study]*len(y))  # Batch vector
...
X_for_combat = X_uni.values   # features × samples
combat = Combat()
X_combat_corrected = combat.fit_transform(X_for_combat, batch_labels)
X_combat_corrected = pd.DataFrame(X_combat_corrected, columns=X_uni.columns)
...
base_preproc = Pipeline([
    ("imputer", rf_imputer),
    ("log1p", FunctionTransformer(lambda Z: np.log1p(np.clip(Z, a_min=-0.999999, a_max=None)), validate=False)),
...
for name, model in models.items():
    ...
    for train_idx, test_idx in cv.split(X_combat_corrected, y):
        X_train, X_test = X_combat_corrected.iloc[train_idx].values, X_combat_corrected.iloc[test_idx].values
...
plt.legend()
plt.tight_layout()
plt.savefig("/Users/user/Downloads/Pooled_Multiclass_ROC.png", dpi=1200)
plt.show()
...
metrics_df.to_csv("/Users/user/Downloads/Pooled_Multiclass_Model_Metrics.csv", index=False)
